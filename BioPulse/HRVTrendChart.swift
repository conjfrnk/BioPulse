...
struct HRVTrendChart: View {
    ...
    var body: some View {
        if dailyHRV.isEmpty {
            ...
            let dateKeys = dailyHRV.keys.sorted()
            let minVal = dailyHRV.values.min() ?? 30
            let maxVal = dailyHRV.values.max() ?? 80
            let yLo = max(0, minVal - 5)
            let yHi = maxVal + 5
            let avgHRV = dailyHRV.values.reduce(0, +) / Double(dailyHRV.count)
            ...
            VStack(alignment: .leading) {
                ...
                Chart {
                    ForEach(dateKeys, id: \.self) { d in
                        LineMark(
                            x: .value("Date", d),
                            y: .value("HRV", dailyHRV[d] ?? 0)
                        )
                    }
                    RuleMark(y: .value("Average HRV", avgHRV))
